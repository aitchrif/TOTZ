(() => {
  ...
  document.getElementById("discordState").textContent = discord || "Waiting for Discord";
  ...
  const post = async (route, body) => {
    const response = await fetch(`/api/discord-link/${route}`, {
      method: "POST",
  ...
  if (!session) {
    ...
    setStatus("Start from the LINK WALLET button in the TOTZ Holder Hub or raffle.", "error");
    return;
  }
  ...
  button.addEventListener("click", async () => {
    ...
    try {
      ...
      setStatus("Discord and wallet linked successfully. You can return to Discord.", "ok");
