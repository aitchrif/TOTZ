(() => {
  ...
  let tokenPolicyConfirmed = false;
  ...
  function canDeploy() {
    return canReview() && tokenPolicyConfirmed && reviewConfirmed && !deploymentStarted;
  }

  function resetReview(message='A setting changed. Review the immutable launch details again before deployment.') {
    tokenPolicyConfirmed = false;
    reviewConfirmed = false;
    const policyAck = $('tokenPolicyAck');
  ...
  function refreshReview() {
    ...
    const policyAck = $('tokenPolicyAck');
    if (policyAck) policyAck.disabled = !canReview() || deploymentStarted;
    const ack = $('reviewAck');
    if (ack) ack.disabled = !canReview() || !tokenPolicyConfirmed || deploymentStarted;
    const btn = $('deployBtn');
    ...
    else if (!tokenPolicyConfirmed) reviewStatus('Confirm the Standard ERC-20 policy. Rebasing, reflection and fee-on-transfer tokens are not supported by FORGE V1.','warn');
    else if (!reviewConfirmed) reviewStatus('Token policy confirmed ✓ Review the immutable values, then tick the final deployment confirmation.','ok');
  ...
  async function verifyToken() {
    ...
    try {
      ...
      if (packageData && decimals !== expectedDecimals) {
        ...
        tokenStatus(`On-chain token verified ✓ ${symbol} · ${decimals} decimals · ${short(address)}${bal} · Standard ERC-20 policy confirmation still required.`,'ok');
      }
    } catch (e) {
      tokenMeta = null;
  ...
  function install() {
    ...
    $('tokenPolicyAck').addEventListener('change',e=>{
      tokenPolicyConfirmed = Boolean(e.target.checked) && canReview();
      reviewConfirmed = false;
      if ($('reviewAck')) $('reviewAck').checked = false;
      refreshReview();
    });
    $('reviewAck').addEventListener('change',e=>{reviewConfirmed=Boolean(e.target.checked)&&canReview()&&tokenPolicyConfirmed;refreshReview();});

    deployBtn.addEventListener('click',e=>{
      if (!canDeploy()) {
        e.preventDefault();e.stopImmediatePropagation();
        reviewStatus('Deployment is locked until the package, token, sponsor, deadline and Standard ERC-20 policy are verified and both confirmations are checked.','error');
    ...
    const deployObserver = new MutationObserver(()=>{
      ...
      if (text.includes('deployed and verified')) {
        ...
        tokenPolicyConfirmed = false;
        reviewConfirmed = false;
        if ($('tokenPolicyAck')) $('tokenPolicyAck').checked = false;
        if ($('reviewAck')) $('reviewAck').checked = false;
        refreshReview();
      }
    });
    ...
    if (window.ethereum?.on) window.ethereum.on('accountsChanged',async()=>{
      ...
      resetReview('Connected wallet changed. Reconfirm the sponsor, token policy and immutable review.');
    });
