...
const report = {
  ...
  abiMissingFromBuild: diffSet(legacyAbi, builtAbi),
  abiExtraInBuild: diffSet(builtAbi, legacyAbi),
};
report.abiSurfaceExact = report.abiMissingFromBuild.length === 0 && report.abiExtraInBuild.length === 0;
report.fullArtifactIdentityExact =
...
report.deployedRuntimeReconstructed =
...
console.log('FORGE CLAIM DEPLOYED-RUNTIME RECONSTRUCTION REPORT');
...
if (!report.normalizedCoreExact) throw new Error('Deployed runtime executable core does not match the approved legacy artifact.');
if (report.runtimeBytes !== report.legacyRuntimeBytes) throw new Error('Deployed runtime bytecode length differs from the approved legacy artifact.');

if (!report.initPrefixExact) {
  console.warn('PROVENANCE LIMITATION: constructor/init bytecode is not an exact reconstruction. This source must not be claimed as the exact historical deployment source.');
}
if (!report.fullArtifactIdentityExact) {
  console.warn('METADATA LIMITATION: compiler metadata/full artifact identity is not byte-for-byte identical to the legacy artifact.');
}

console.log('FORGE CLAIM RUNTIME: PASS · ABI, immutable positions, runtime length, and normalized deployed executable core match the approved legacy artifact. Historical constructor/source provenance remains unproven.');
