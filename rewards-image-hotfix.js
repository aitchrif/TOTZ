(() => {
  const gateways = ['https://dweb.link/ipfs','https://gateway.pinata.cloud/ipfs','https://ipfs.io/ipfs'];
  function parseIpfsUrl(value) {
    ...
    if (raw.startsWith('ipfs://')) {
      ...
      return slash === -1 ? { cid: rest, path: '' } : { cid: rest.slice(0, slash), path: rest.slice(slash + 1) };
    }
    try {
      const url = new URL(raw, location.href);
      const subdomain = url.hostname.match(/^([a-z0-9]+)\.ipfs\./i);
      if (subdomain) return { cid: subdomain[1], path: url.pathname.replace(/^\/+/, '') };
      const parts = url.pathname.split('/').filter(Boolean);
      const ipfsIndex = parts.findIndex((part) => part.toLowerCase() === 'ipfs');
      if (ipfsIndex >= 0 && parts[ipfsIndex + 1]) return { cid: parts[ipfsIndex + 1], path: parts.slice(ipfsIndex + 2).join('/') };
    } catch (_) {}
    return null;
  }
  ...
  function prepare(img) {
    if (!(img instanceof HTMLImageElement) || !img.closest('.prize-media') || img.dataset.ipfsPrepared === '1') return;
    const parsed = parseIpfsUrl(img.getAttribute('src'));
    img.dataset.ipfsPrepared = '1';
    img.referrerPolicy = 'no-referrer';
    img.decoding = 'async';
    if (!parsed?.cid) {
      img.addEventListener('error', () => { img.style.display = 'none'; addPlaceholder(img); }, { once: true });
      return;
    }
    const media = img.closest('.prize-media');
    ...
    gateways.forEach((base) => {
      ...
      probe.onerror = () => { failed += 1; if (!settled && failed === gateways.length) { settled = true; img.style.display = 'none'; addPlaceholder(img); } };
      probe.src = url;
    });
    setTimeout(() => { if (!settled) { settled = true; img.style.display = 'none'; addPlaceholder(img); } }, 8000);
  }
